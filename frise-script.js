...
function drawPeriods() {
  ...
  periods.forEach(p => {
    ...
    div.innerHTML = `
      <div class="period-resize-left"></div>
      <div class="period-resize-right"></div>
    ...
    div.addEventListener('mousedown', (e) => {
      if (e.target.classList.contains('period-resize-left') || e.target.classList.contains('period-resize-right')) {
        return;
      }
    ...
    // Resize handles
    const resizeLeft = div.querySelector('.period-resize-left');
    const resizeRight = div.querySelector('.period-resize-right');
...
function handleResize(e) {
  ...
  if (resizingItem.type === 'event') {
    ...
    render();
    saveToLocalStorageSilent();
  } else if (resizingItem.type === 'period-left') {
    const p = resizingItem.item;
    const newYear = xToYear(m.x);
    if (newYear < parseInt(p.endYear)) {
      p.startYear = String(newYear);
      render();
      saveToLocalStorageSilent();
    }
  } else if (resizingItem.type === 'period-right') {
    const p = resizingItem.item;
    const newYear = xToYear(m.x);
    if (newYear > parseInt(p.startYear)) {
      p.endYear = String(newYear);
      render();
      saveToLocalStorageSilent();
    }
  }
...
async function deleteSelectedItem() {
  ...
  const confirmed = await showConfirm(
    'Supprimer ?',
    `Voulez-vous vraiment supprimer ${type} ?`
  );
...
function saveToFile() {
  try {
    const data = { events, periods, artists, settings, version: '2.1' };
